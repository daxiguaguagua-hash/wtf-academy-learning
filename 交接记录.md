...
## 老师作业纠偏：角色互换后形成的 DEX 新规划

### 当前判断

| 项目 | 结论 |
|---|---|
| 关键纠偏 | 老师说的是 `DEX`，不是 `DES` |
| 老师最看重 | `下单 / 平仓 / 杠杆` 的理解和可演示作品 |
| 交付形态 | 必须有 `可运行前端 + 合约演示 + 测试网部署` |
| 求职目标 | 面向 `交易所前端岗位` 的作品，不只是课堂作业 |
| 当前最大阻塞 | `Sepolia` 测试网部署经验不足 |

### 角色互换后已确认的产品边界

| 项目 | 选择 |
|---|---|
| 作品方向 | `MinimalPerpDemo / Minimal Long Position Demo` |
| 业务范围 | 先只做 `Long` |
| 前端框架 | `Next.js` |
| 钱包 | `MetaMask` |
| 价格机制 | 管理员手动调价格，作为教学版 Oracle |
| 历史数据 | 需要保留历史记录 |
| 页面风格 | 默认暗色，可切亮色 |
| UI 生成策略 | `Stitch + Gemini` 出视觉方案，我们负责实现 |

### 当前最优推进顺序

```mermaid
flowchart LR
A[先打掉部署恐惧] --> B[把现有简单合约部署到 Sepolia]
B --> C[前端临时连上测试网]
C --> D[建立测试网操作信心]
D --> E[正式启动 MinimalPerpDemo]
```
...
| 原因 | 说明 |
|---|---|
| 当前最大短板不是业务理解 | 用户对 `下单 / 平仓 / 杠杆` 已有基础概念 |
| 最大短板是链路经验 | `RPC / 测试币 / 部署 / Etherscan` 这条链还没真正跑通 |
| 先做部署演练更稳 | 能先解决最大心理阻塞，避免正式作品阶段卡死 |

### 当前执行策略

1. 先拿仓库里已有的简单合约做一次 `Sepolia` 部署演练。
2. 前端临时连上测试网，确认“页面点击 -> 钱包签名 -> 链上结果”闭环。
3. 部署链路跑通后，再新建正式作品：
   - 合约：`MinimalPerpDemo`
   - 前端：`Next.js`
   - 页面：开仓、仓位、价格控制、平仓、历史记录
...
| `Sepolia RPC URL` | 待申请 |
| `Sepolia ETH` | 当前没有 |
| `Etherscan` 使用认知 | 已初步厘清 |
| `opencli` 浏览器辅助 | 当前 VS Code 环境不可用，准备切到 `Codex Desktop` 后继续 |

### 切到 Codex Desktop 后的第一优先级

```mermaid
flowchart LR
A[进入 Codex Desktop] --> B[结合截图或 opencli 看浏览器]
B --> C[申请 RPC]
C --> D[申请 Sepolia ETH]
D --> E[部署现有合约]
```
...
1. 不要继续沿着 `DES` 路线推进作业主线。
2. 默认把本次作业主线改成：
   - `DEX`
   - 核心概念：`下单 / 平仓 / 杠杆`
   - 作品目标：`交易所前端求职作品`
3. 先解决测试网部署阻塞，再推进正式产品开发。
4. 若用户切到 `Codex Desktop`，优先利用更适合浏览器协作的能力，指导其完成：
   - `Alchemy/Infura` RPC 申请
   - `Sepolia Faucet` 领测试币
   - `Foundry` 部署
   - `Etherscan` 查看结果
