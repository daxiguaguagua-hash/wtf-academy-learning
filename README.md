...
| 时间 | 事件 |
|---|---|
| `2026-04-08` | 当前开始进入冲刺规划 |
| `2026-04-25` | 老师线下课开始 |

当前最优策略不是“先完整学完 `Solidity102 + Solidity103` 再看项目”，而是：

```mermaid
flowchart LR
A[已完成 Solidity101] --> B[选学 102/103 高收益章节]
B --> C[补 DES 相关业务知识]
C --> D[做最小模拟项目]
D --> E[准备 4月25日 线下课/录播]
```
...
| 方案 | 时间成本 | 对 4 月 25 日收益 | 结论 |
|---|---:|---:|---|
| 完整刷完 `102 + 103` | 高 | 中 | 不划算 |
| 只学高相关章节 + 提前补 DES | 中 | 高 | 最优 |
| 直接硬啃老师项目图 | 中高 | 不稳定 | 容易卡住 |

结论：
- `4月25日` 前目标不是“学完全部课程”。
- `4月25日` 前目标是“补齐最影响 DES 项目理解的核心能力”。
...
```mermaid
flowchart TD
A[第1阶段 4/8-4/14] --> B[第2阶段 4/15-4/20]
B --> C[第3阶段 4/21-4/24]
C --> D[4/25 进入线下课或看录播]
```
...
目标：
- 从 `Solidity102` 中补最影响读项目源码的章节。

优先章节：

| 优先级 | 课程 | 主题 | 作用 |
|---|---|---|---|
| 高 | `Solidity102` | `Fallback` | 理解特殊入口函数 |
| 高 | `Solidity102` | `Interact with Contract` | 理解合约之间如何调用 |
| 高 | `Solidity102` | `Call` | 理解低级调用 |
| 高 | `Solidity102` | `Delegatecall` | 理解代理和上下文 |
| 高 | `Solidity102` | `ABI Encoding and Decoding` | 理解参数编码与返回值解码 |
| 高 | `Solidity102` | `Hash` | 理解哈希、签名、证明基础 |
| 高 | `Solidity102` | `Function Selector` | 理解函数选择器 |
| 高 | `Solidity102` | `Try Catch` | 理解跨合约失败处理 |

### 第 2 阶段：`4/15 - 4/20`

目标：
- 从 `Solidity103` 中补最接近 DES 项目结构的章节。

优先章节：

| 优先级 | 课程 | 主题 | 作用 |
|---|---|---|---|
| 高 | `Solidity103` | `ERC20` | 理解资金、余额、授权 |
| 高 | `Solidity103` | `Digital Signature` / `EIP712` | 理解签名授权 |
| 高 | `Solidity103` | `ERC4626` | 理解 Vault / shares |
| 高 | `Solidity103` | `Proxy Contract` | 理解代理模式 |
| 高 | `Solidity103` | `Transparent Proxy` | 理解常见升级代理 |
| 高 | `Solidity103` | `UUPS` | 理解升级控制 |
| 高 | `Solidity103` | `Multisignature Wallet` | 理解多签权限 |

### 第 3 阶段：`4/21 - 4/24`

目标：
- 不再继续横向刷课，转向 DES 项目预习。

必补主题：

| 主题 | 必须搞懂什么 |
|---|---|
| `Vault / LP / shares` | 存入、铸份额、赎回、净值 |
| `Oracle` | 价格从哪里来，为什么不能手填 |
| `Liquidation` | 什么时候爆仓，谁触发清算 |
| `Permission` | 多签、Timelock、升级权限 |
| `Frontend flow` | 读状态、发交易、监听事件、刷新页面 |
...
|---|---|
| `1` | `Solidity102` 高优先章节 |
| `2` | `Solidity103` 里的 `ERC20 / EIP712 / ERC4626 / Proxy / UUPS / Multisig` |
| `3` | DES 业务模型 |
| `4` | 其余 `103` 章节 |

不建议在 `4月25日` 前优先投入大量时间的方向：

| 内容 | 原因 |
|---|---|
| `NFT` 相关专题 | 对这次 DES 项目直接帮助较小 |
| 过多拍卖类合约 | 与本次目标不够贴合 |
| 纯刷章节不做代码验证 | 记不住，也跟不上项目 |
